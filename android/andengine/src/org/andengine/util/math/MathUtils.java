...
public final class MathUtils {
    ...
    private static float degToRad(final float pDegree) {
    ...
    private static float arraySum(final float[] pValues) {
        float sum = 0;
        final int valueCount = pValues.length;
    ...
    public static void revertScaleAroundCenter(final float[] pVertices, final float pScaleX, final float pScaleY, final float pScaleCenterX, final float pScaleCenterY) {
        MathUtils.scaleAroundCenter(pVertices, 1 / pScaleX, 1 / pScaleY, pScaleCenterX, pScaleCenterY);
    }
